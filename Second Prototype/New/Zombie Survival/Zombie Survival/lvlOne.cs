...
namespace Zombie_Survival
{
    public partial class lvlOne : Form
    {
        ...
        TimeSpan timeleft1 = TimeSpan.FromMinutes(1);
        TimeSpan timeleft2 = TimeSpan.FromMinutes(2);
        TimeSpan timeleft3 = TimeSpan.FromMinutes(3);//Used for countdown

        int WaveNo = 1;//keeps track of wave number
        ...
        private void lvlOne_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (gameOver == false)//makes sure cant move when dead
            {
                if (e.KeyCode == Keys.Left)
                {
                    goLeft = true;
                    facing = "left";
                    pboPlayer.Image = Properties.Resources.leftCampaign;//changes player image to left one
                }
                //right
                if (e.KeyCode == Keys.Right)
                {
                    goRight = true;
                    facing = "right";
                    pboPlayer.Image = Properties.Resources.rightCampaign;//changes player image to right one
                }
                //up
                if (e.KeyCode == Keys.Up)
                {
                    goUp = true;
                    facing = "up";
                    pboPlayer.Image = Properties.Resources.upCampaign;//changes player image to up one
                }
                //down
                if (e.KeyCode == Keys.Down)
                {
                    goDown = true;
                    facing = "down";
                    pboPlayer.Image = Properties.Resources.downCampaign;//changes player image to down one
                }
            }
            if (gameOver == true)
            {
        ...
        private Rectangle frmGameOriginal, lblKillsOriginal, lblHealthOriginal, lblAmmoOriginal,
            ...
            pboLabelKillsOriginal, pboLabelHealthOriginal, lblCountdownOriginal, lblWaveOriginal;

        private void tmrCountdown_Tick(object sender, EventArgs e)
        {
            if (WaveNo == 1)
            {
                timeleft1 = timeleft1.Subtract(TimeSpan.FromSeconds(1));
                lblCountdown.Text = "Time left: " + timeleft1.ToString("mm\\:ss");
            }
            else if(WaveNo == 2)
            {
                timeleft2 = timeleft2.Subtract(TimeSpan.FromSeconds(1));
                lblCountdown.Text = "Time left: " + timeleft2.ToString("mm\\:ss");
            }
            else if (WaveNo == 3)
            {
                timeleft3 = timeleft3.Subtract(TimeSpan.FromSeconds(1));
                lblCountdown.Text = "Time left: " + timeleft3.ToString("mm\\:ss");
            }
        }

        private void lvlOne_Resize(object sender, EventArgs e)
        {
            ...
            resizeControl(lblWaveOriginal, lblWave);//new control
            resizeControl(lblCountdownOriginal, lblCountdown);//new control
        ...
        {
            tmrGameTimer1.Stop();
            tmrCountdown.Stop();//stops the game and countdown from running too
            ...
            lblCountdownOriginal = new Rectangle(lblCountdown.Location.X, lblCountdown.Location.Y, lblCountdown.Width, lblCountdown.Height);
            lblWaveOriginal = new Rectangle(lblWave.Location.X, lblWave.Location.Y, lblWave.Width, lblWave.Height);
            ...
            foreach (Control x in this.Controls)
            {
                ...
                {//moves zombie toward player pbo
                    ...
                    if (((PictureBox)x).Left > pboPlayer.Left)
                    {
                        ...
                        ((PictureBox)x).Image = Properties.Resources.zleftCampaign;//make zombie face left
                    }
                    if (((PictureBox)x).Top > pboPlayer.Top)
                    {
                        ((PictureBox)x).Top -= zombieSpeed; //moves toward left of player
                        ((PictureBox)x).Image = Properties.Resources.zupCampaign;//make zombie face left
                    }
                    if (((PictureBox)x).Left < pboPlayer.Left)
                    {
                        ((PictureBox)x).Left += zombieSpeed; //moves toward left of player
                        ((PictureBox)x).Image = Properties.Resources.zrightCampaign;//make zombie face left
                    }
                    if (((PictureBox)x).Top < pboPlayer.Top)
                    {
                        ((PictureBox)x).Top += zombieSpeed; //moves toward left of player
                        ((PictureBox)x).Image = Properties.Resources.zdownCampaign;//make zombie face left
                    }
                }
            }
        }
        ...
        {//used when game starts or is restarted
            playerHealth = 100;
            gameOver = false;
            pboPlayer.Image = Properties.Resources.upCampaign;//makes sure player is facing up
            ammo = 10;
            score = 0;
            tmrGameTimer1.Start();
            tmrCountdown.Start();
        ...
        class Zombie
        {
            ...
            {//when called it makes a zombie of corresponding level
                ...
                zombie.image.Top = rnd.Next(0, frmGame.ActiveForm.Height - zombie.image.Height - 80);//sets y to random value
